...
//			if ( this.ssoConfigurationList != null ) {
	...
	private void handleError(IMxRuntimeResponse response, Exception e) {
		...
		//edit code - start // e.getFeedbackMessage.UserId
		_logNode.error("Unable to validate Response, see SAMLRequest overview for detailed response. Error: " + e.getFeedbackMessage.UserId, e);
		String UserId = e.getFeedbackMessage.UserId;
		String ErrorStatus = e.getFeedbackMessage.ErrorStatus;
		//end code
		...
		try {
			...
			} else if (ErrorStatus=="E002") {
				this.engine.mergeTemplate("templates/saml2-error-security.vm", "UTF-8", ctx, writer);
			}
